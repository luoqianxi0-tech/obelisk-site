...
import { 
  Shield, Users, Layers, Plus, Upload, FileJson, 
  Trash2, X, Globe, Wrench, Palette, BookOpen
} from 'lucide-react'
    ...
    <div className="min-h-screen px-4 sm:px-6 py-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center gap-3 mb-8">
          <div className="w-10 h-10 rounded-xl bg-obelisk-line flex items-center justify-center">
            <Shield className="w-5 h-5 text-white" />
          </div>
          <div>
            <h1 className="section-title">管理面板</h1>
            <p className="text-xs text-obelisk-textMuted">ADMIN ACCESS GRANTED</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <GlassCard>
            <div className="flex items-center gap-3">
              <Users className="w-5 h-5 text-obelisk-textMuted" />
              <div>
                <p className="text-2xl font-bold">{stats.users}</p>
                <p className="text-xs text-obelisk-textMuted">用户</p>
              </div>
            </div>
          </GlassCard>
          <GlassCard>
            <div className="flex items-center gap-3">
              <BookOpen className="w-5 h-5 text-obelisk-textMuted" />
              <div>
                <p className="text-2xl font-bold">{stats.posts}</p>
                <p className="text-xs text-obelisk-textMuted">帖子</p>
              </div>
            </div>
          </GlassCard>
          <GlassCard>
            <div className="flex items-center gap-3">
              <Layers className="w-5 h-5 text-obelisk-textMuted" />
              <div>
                <p className="text-2xl font-bold">{stats.resources}</p>
                <p className="text-xs text-obelisk-textMuted">资源</p>
              </div>
            </div>
          </GlassCard>
          <GlassCard>
            <div className="flex items-center gap-3">
              <Palette className="w-5 h-5 text-obelisk-textMuted" />
              <div>
                <p className="text-2xl font-bold">{stats.designs}</p>
                <p className="text-xs text-obelisk-textMuted">设计</p>
              </div>
            </div>
          </GlassCard>
        </div>
        ...
        <div className="flex gap-2 mb-6 overflow-x-auto scrollbar-hide">
          {tabs.map(tab => (
            <button
              key={tab.id}
              onClick={() => handleTabChange(tab.id === 'index_items' ? 'index' : tab.id === 'design_items' ? 'design' : tab.id)}
              className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium transition-all whitespace-nowrap ${
                activeTab === (tab.id === 'index_items' ? 'index' : tab.id === 'design_items' ? 'design' : tab.id)
        ...
        <div className="flex gap-2 mb-4">
          ...
          <button onClick={() => setShowImportModal(true)} className="btn-secondary text-sm flex items-center gap-2">
        ...
        {loading ? (
          ...
          <GlassCard>
            <div className="text-center py-12">
              <Layers className="w-12 h-12 mx-auto mb-3 text-obelisk-border" />
              <p className="text-obelisk-textMuted">暂无数据</p>
            </div>
          </GlassCard>
        ) : (
              ...
              <GlassCard key={item.id} className="flex items-center justify-between">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="tag text-xs">{item.category || item.type || 'general'}</span>
                ...
                <button onClick={() => handleDelete(item.id)} className="p-2 hover:bg-red-50 rounded-lg transition-colors ml-4">
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/30 backdrop-blur-sm" onClick={() => setShowAddModal(false)}>
          <div className="glass-panel rounded-2xl p-6 w-full max-w-md" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold">添加条目</h3>
              <button onClick={() => setShowAddModal(false)}><X className="w-5 h-5" /></button>
            </div>
            <div className="space-y-3">
              <input type="text" value={newTitle} onChange={e => setNewTitle(e.target.value)} placeholder="名称" className="input-field" />
              ...
              <input type="text" value={newTags} onChange={e => setNewTags(e.target.value)} placeholder="标签" className="input-field" />
              <div className="flex gap-3">
                <button onClick={() => setShowAddModal(false)} className="btn-secondary flex-1">取消</button>
                <button onClick={handleAdd} disabled={!newTitle.trim() || !newUrl.trim()} className="btn-primary flex-1">添加</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {showImportModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/30 backdrop-blur-sm" onClick={() => setShowImportModal(false)}>
          <div className="glass-panel rounded-2xl p-6 w-full max-w-lg" onClick={e => e.stopPropagation()}>
            ...
            <select value={importTarget} onChange={e => setImportTarget(e.target.value)} className="input-field mb-3">
              <option value="resources">聚合资源</option>
              <option value="index">索引</option>
              <option value="design">设计</option>
            </select>
            <p className="text-xs text-obelisk-textMuted mb-2">格式：名称, URL, 分类, 描述, 标签</p>
