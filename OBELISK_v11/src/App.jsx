        ...
        <div className="min-h-screen flex items-center justify-center p-6">
          <div className="text-center">
            ...
            <div className="flex justify-center gap-1 mb-4">
              {[0,1,2].map(i => <div key={i} className="w-1.5 h-1.5 bg-black/30 rounded-full animate-pulse" style={{ animationDelay: `${i*150}ms` }} />)}
            </div>
            <div className="text-xs text-black/40 tracking-wide">Loading...</div>
          </div>
        </div>
      ) : (
        <Routes>
          <Route element={<Layout />}>
            ...
            <Route path="*" element={<div className="max-w-xl mx-auto px-4 py-24 text-center"><div className="text-5xl font-light tracking-[0.2em] mb-4">404</div><div className="h-[1px] w-16 bg-black/20 mx-auto mb-6" /><p className="text-black/50">Page not found</p></div>} />
          </Route>
        </Routes>
      )}
      <AudioPlayer />
      {!firebaseConfigured && splashDone && (
        <div className="fixed bottom-4 left-4 right-4 md:left-auto md:right-6 md:max-w-sm z-40">
          <div className="glass rounded-lg p-4 border-l-2 border-black/40">
            <div className="text-xs font-medium tracking-wide mb-1">Firebase Not Configured</div>
            <p className="text-xs text-black/50 leading-relaxed mb-2">Add Firebase environment variables to enable full functionality.</p>
            {firebaseMissing?.length > 0 && <p className="text-[10px] font-mono text-black/40 break-all mb-2">{firebaseMissing.map(k => `VITE_FIREBASE_${k.replace(/([A-Z])/g,'_$1').toUpperCase()}`).join(', ')}</p>}
            {initError && initError !== 'FIREBASE_NOT_CONFIGURED' && <p className="text-[10px] font-mono text-black/40 break-all">{initError}</p>}
          </div>
        </div>
      )}
    </>
