...
import { MessageSquare, Target, FolderGit2, BookOpen, Wrench, Palette, ArrowUpRight } from 'lucide-react';
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12 md:py-20">
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-24">
        ...
        <p className="text-lg text-black/40 max-w-lg mx-auto leading-relaxed tracking-wide">
          {t('home.subtitle')}
        </p>
      </motion.div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-24">
        {sections.map((section, i) => (
          <Link key={section.path} to={section.path}>
            <GlassCard delay={i * 0.08} className={`h-full group ${section.border}`}>
          ...
          {featured.map((tool, i) => (
            <GlassCard key={tool.title} delay={i * 0.05} className="flex items-center justify-between group">
              ...
              <a href={tool.url} target="_blank" rel="noopener noreferrer"
                className="text-xs border border-black/10 px-3 py-1.5 hover:bg-black hover:text-white transition-colors shrink-0 ml-4">
                {t('common.visit')}
              </a>
            </GlassCard>
          ))}
